...
window.leafletInterop = {
   ...
   * @param {boolean} fitBounds - Whether to adjust the map view to fit the circle
   * @returns {boolean} Success status
   */
  updateSearchRadius: function (mapId, lat, lng, radiusKm, fitBounds = false) {
    try {
      const mapInstance = this.maps[mapId];
      if (!mapInstance) {
        console.error("Map not found:", mapId);
      ...
      // Optionally fit the map to show the entire circle
      if (fitBounds) {
  ...
  createGroupPopupContent: function (group) {
    ...
    let websiteHtml = "";
    if (group.websiteUrl) {
      // Validate URL protocol to prevent XSS via javascript: or other dangerous schemes
   ...
   * @param {string} mapId - The map instance ID
   * @returns {object|null} Object with {latitude, longitude, zoom}, or null if map not found
   */
